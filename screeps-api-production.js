class ScreepsAPI {
    ...
    setServerUrl(url) {
        if (url.includes('localhost') || url.includes('127.0.0.1')) {
            ...
            // Official server - allow both proxy and direct access
            const useDirectAccess = localStorage.getItem("screeps_use_direct_access") === 'true';
            if (useDirectAccess) {
                this.baseUrl = "https://screeps.com/api/";
                this.useProxy = false;
            } else {
    ...
    enableDirectAccess(enable = true) {
        localStorage.setItem("screeps_use_direct_access", enable.toString());
        if (this.baseUrl.includes('screeps.com')) {
            this.setServerUrl('https://screeps.com/api/');
        }
    }
